...
class FakeRepository implements ResearchModeRepository {
  ...
  // 27.2/28.6 merge-gate fix: `ResearchModeClaim`/`ResearchModeRelationship`
  // deliberately carry no `status`/`hidden` field (the real DB-backed
  // repository never SELECTs them — filtering happens in the WHERE clause,
  // not the returned shape). These two sets let a test simulate "this claim
  // is superseded"/"this relationship is hidden" without widening either
  // public interface, mirroring how the real `clusterMemberClaims`/
  // `clusterRelationships` now apply the same status/hidden guard every
  // other `claim*`/relationship read in this repository already applied.
  nonActiveClaimIds = new Set<string>();
  nonActiveRelationshipIds = new Set<string>();
  ...
  async clusterMemberClaims(_userId: string, clusterId: string) {
    ...
    return ids
      .filter((id) => !this.nonActiveClaimIds.has(id))
      .map((id) => this.claims.get(id))
      .filter((c): c is ResearchModeClaim => Boolean(c));
  }
  async clusterRelationships(_userId: string, clusterId: string) {
    const ids = new Set(this.clusterRelationshipIds.get(clusterId) ?? []);
    return this.relationships.filter((r) => ids.has(r.id) && !this.nonActiveRelationshipIds.has(r.id));
  }
}
...
describe("retrieveDebateMap", () => {
  ...
  // 27.2/28.6 merge-gate fix: `clusterMemberClaims`/`clusterRelationships`
  // in `createDbResearchModeRepository` were the only two reads in this
  // repository missing the `status = 'active' AND hidden = false` guard
  // every other `claim*`/relationship read already applies — a `map_debate`
  // answer could surface a withdrawn or superseded claim/relationship
  // reached only via cluster membership, not a direct id lookup.
  it("excludes a superseded/hidden claim from cluster membership, even though it's a real member", async () => {
    const repo = new FakeRepository();
    const a1 = claim({ id: "a1", workId: "work-a" });
    const b1 = claim({ id: "b1", workId: "work-b" });
    repo.claims.set(a1.id, a1);
    ...
    repo.clusters.set("cluster-1", { id: "cluster-1", name: "Akrasia debate", researchQuestion: "Q", description: "D" });
    repo.clusterMembers.set("cluster-1", ["a1", "b1"]);
    repo.nonActiveClaimIds.add("b1");

    const claims = await repo.clusterMemberClaims(USER, "cluster-1");
    expect(claims.map((c) => c.id)).toEqual(["a1"]);
  });

  it("excludes a superseded/hidden relationship from cluster membership, even though it's a real member", async () => {
    const repo = new FakeRepository();
    const a1 = claim({ id: "a1", workId: "work-a" });
    const b1 = claim({ id: "b1", workId: "work-b" });
    repo.claims.set(a1.id, a1);
    ...
    repo.relationships.push(relationship({ id: "rel-1", claimLoId: "a1", claimHiId: "b1", valence: "contradiction" }));
    repo.clusterRelationshipIds.set("cluster-1", ["rel-1"]);
    repo.nonActiveRelationshipIds.add("rel-1");

    const relationships = await repo.clusterRelationships(USER, "cluster-1");
