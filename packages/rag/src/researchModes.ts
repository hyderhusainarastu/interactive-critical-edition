  ...
  return {
    ...
    async clusterMemberClaims(userId, clusterId) {
      ...
      return db
        ...
        .innerJoin(works, eq(researchClaims.workId, works.id))
        .where(and(eq(debateClusterMembers.clusterId, clusterId), eq(researchClaims.userId, userId))) as unknown as Promise<
        ResearchModeClaim[]
      >;
    },
    ...
    async clusterRelationships(userId, clusterId) {
      ...
      return db
        ...
        .where(
          and(eq(debateClusterRelationships.clusterId, clusterId), eq(claimRelationships.userId, userId)),
        ) as unknown as Promise<ResearchModeRelationship[]>;
    },
  };
}
