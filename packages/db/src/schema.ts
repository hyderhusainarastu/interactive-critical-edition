...
export const researchRevisions = pgTable(
  "research_revision",
  {
    ...
    // 0043 (this migration; the 0042 Evidence Chamber pair is a parallel,
    // not-yet-landed lane and is skipped here): the fourth and fifth of the
    // seven typed object FK columns — same forward-reference pattern
    // (`researchHypotheses`/`researchGaps` are declared later in this
    // incrementally-grown file, at the very end, after
  ...
  (t) => [
     ...
     * type's FK. Widened again here (0043, this migration, landing ahead of
     * the parallel 0042 Evidence Chamber lane) to add the `'hypothesis'`/
     * `'gap'` branches and pin the two new typed FK columns
     * (`research_hypothesis_id`/`research_gap_id`) on every branch the same
     * way. `'chamber'`/`'position'` (0042's own values) remain in the
     * catch-all branch below, unsatisfiable until that lane adds its own
     * columns — the same staging discipline this whole class doc comment
     * describes.
     */
    check(
      "research_revision_typed_target",
      sql`(${t.objectType} = 'claim' AND ${t.researchClaimId} IS NOT NULL AND ${t.claimRelationshipId} IS NULL AND ${t.debateClusterId} IS NULL AND ${t.researchHypothesisId} IS NULL AND ${t.researchGapId} IS NULL)
        OR (${t.objectType} = 'relationship' AND ${t.claimRelationshipId} IS NOT NULL AND ${t.researchClaimId} IS NULL AND ${t.debateClusterId} IS NULL AND ${t.researchHypothesisId} IS NULL AND ${t.researchGapId} IS NULL)
        OR (${t.objectType} = 'cluster' AND ${t.debateClusterId} IS NOT NULL AND ${t.researchClaimId} IS NULL AND ${t.claimRelationshipId} IS NULL AND ${t.researchHypothesisId} IS NULL AND ${t.researchGapId} IS NULL)
        OR (${t.objectType} = 'hypothesis' AND ${t.researchHypothesisId} IS NOT NULL AND ${t.researchClaimId} IS NULL AND ${t.claimRelationshipId} IS NULL AND ${t.debateClusterId} IS NULL AND ${t.researchGapId} IS NULL)
        OR (${t.objectType} = 'gap' AND ${t.researchGapId} IS NOT NULL AND ${t.researchClaimId} IS NULL AND ${t.claimRelationshipId} IS NULL AND ${t.debateClusterId} IS NULL AND ${t.researchHypothesisId} IS NULL)
        OR (${t.objectType} NOT IN ('claim', 'relationship', 'cluster', 'hypothesis', 'gap') AND ${t.researchClaimId} IS NULL AND ${t.claimRelationshipId} IS NULL AND ${t.debateClusterId} IS NULL AND ${t.researchHypothesisId} IS NULL AND ${t.researchGapId} IS NULL)`,
    ),
