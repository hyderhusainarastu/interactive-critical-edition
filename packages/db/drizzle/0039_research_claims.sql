...
CREATE TABLE "research_claim" (
	...
	-- Added per adversarial-verifier finding, before this migration was ever
	-- applied to production (still local-only at the time): tightens the
	-- grounded invariant above so an 'unanchored' claim can never carry a
	-- stale text_block_id alongside it.
	CONSTRAINT "research_claim_unanchored_no_block" CHECK ("research_claim"."anchor_state" <> 'unanchored' OR "research_claim"."text_block_id" IS NULL),
