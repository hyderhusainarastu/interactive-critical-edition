  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6" aria-labelledby="writer-title">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3"><button type="button" className="app-control text-sm text-[var(--color-text-muted)] underline" onClick={loadArchived} disabled={loadingArchived}>{loadingArchived ? "Loading…" : "Show archived projects"}</button><button type="button" className="app-control rounded bg-[var(--color-accent-ink)] px-4 py-2 text-sm font-medium text-[var(--color-background)] disabled:opacity-50" onClick={createProject} disabled={creating}>{creating ? "Creating…" : "New project"}</button></div>
      </div>
      <ul className="mt-7 grid gap-3 sm:grid-cols-2" aria-label="Writing projects">
        ...
        {projects.map((project) => <li key={project.id}><Link href={`/writer/${project.id}`} className="app-control block rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4"><h2 className="font-medium">{project.title}</h2><p className="mt-1 text-sm text-[var(--color-text-muted)]">{project.documentCount} {project.documentCount === 1 ? "document" : "documents"} · updated {new Date(project.updatedAt).toLocaleDateString()}</p></Link></li>)}
