...
export default async function ResearchProjectMonitorsPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [monitors, hits, jobRequests] = await Promise.all([
    ...
    listResearchJobRequestsForUserMonitors(session.user.id, projectId),
  ]);
  return <MonitorsView project={project} initialMonitors={monitors} initialHits={hits} initialJobRequests={jobRequests} />;
}
