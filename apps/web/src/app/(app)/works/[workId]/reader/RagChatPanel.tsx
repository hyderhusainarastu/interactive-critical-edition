...
// Phase 28.6: a narrow, client-safe local mirror of `@ice/rag`'s
// `ResearchMode`/`RESEARCH_MODES`/`RESEARCH_MODE_LABEL` (the same pattern
// `Message`/`Citation` below already use for the server-side `ragData.ts`
// shapes) — NOT imported from `@ice/rag` directly. That package's barrel
// (`src/index.ts`) re-exports `researchModes.ts`'s `createDbResearchModeRepository`,
// whose dynamic `import("@ice/db")` still gets pulled into the client
// bundle's dependency graph for code-splitting purposes even though it's
// never called there, and `@ice/db` imports `postgres`/`pg-boss`, which use
// Node built-ins (`tls`, `util/types`) that don't exist in a browser bundle
// — confirmed as a real `next build` failure before this file switched to a
// local, I/O-free mirror instead.
const RESEARCH_MODES = ["socratic", "find_counterarguments", "explain_disagreement", "map_debate", "find_support"] as const;
type ResearchMode = (typeof RESEARCH_MODES)[number];
const RESEARCH_MODE_LABEL: Record<ResearchMode, string> = {
  socratic: "Socratic",
