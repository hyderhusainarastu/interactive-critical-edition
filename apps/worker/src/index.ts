...
async function main() {
  ...
  // their own lanes (27 synthesis, 28.2/29 corpus import+monitors) replace
  ...
  // consuming it.
  for (const queueName of RESEARCH_QUEUES.filter((name) => name !== QUEUE_EXTRACT_RESEARCH_CLAIMS && name !== QUEUE_ANALYZE_CLAIM_DEBATES)) {
