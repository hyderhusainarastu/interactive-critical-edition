...
export interface ResearchJobRunContext {
   ...
   *  a usage-log row (flushed in batches of 5 and unconditionally in
   *  `finally` — the `analyze.ts` `flushUsageLogs`/`USAGE_FLUSH_BATCH_SIZE`
   *  idiom). Awaited so the batch flush can actually happen inline; never
   *  drops a log entry, only defers when it writes it. */
  logUsage(input: {
    task: string;
    stage: string;
    provider: string;
