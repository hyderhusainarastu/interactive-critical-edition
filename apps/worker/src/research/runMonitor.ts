 ...
 *
 * Idempotent per UTC calendar day via `idempotencyKey`: if the cron fires
 * more than once on the same day (a redelivery, an operator re-triggering
 * it), the SAME in-flight/complete request row is reused rather than
 * duplicated (`research_job_request_inflight_idempotency_unique`'s partial
 * unique — same mechanism `dispatchExtractClaimsJob` relies on).
 */
