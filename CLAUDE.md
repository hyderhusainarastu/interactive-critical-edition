...
| Playwright E2E suite built and passing locally, but not yet wired into CI | The existing `ci.yml` only stands up a Postgres service container; the reader's interactive flows (text selection, highlight creation, split-pane) can only be verified through real browser automation, which needs the worker running too. Wiring this is real, scoped follow-up work (documented in `playwright.config.ts` and here), not an oversight — the suite was still run and verified manually before Phase 3 was marked complete |

Full rationale for every stack choice, including rejected alternatives: plan §4–§6.

## Current Implementation Status

**Phase 0 — Research & Planning: complete. Phase 1 (Foundation): complete. Phase 2 (Upload and Library): complete. Phase 3 (Reader): complete — local and deployed.** Phases 4–8 not started.

- Repo: https://github.com/hyderhusainarastu/interactive-critical-edition (private)
- Checkpoint tags: `phase-0-complete`, `phase-1a-complete`, `phase-1-complete`, `phase-2a-complete`, `phase-2-complete`, `phase-3-complete`
...
- **Database:** Supabase project `interactive-critical-edition` (ref `vlrzvwswippuaitmrujz`, org `jkcecpjinqwpwxfvuylf`, region `us-east-1`), `pgvector` enabled, all migrations through `0003` applied (local and production).
...
- [ ] Phase 4 (Scholarly Analysis) per `docs/architecture/plan.md` §23 — next up.
- [ ] Phases 5–8 per `docs/architecture/plan.md` §23.
- [ ] Not yet built, deferred to their owning phase: `packages/ai-adapters`, `packages/bibliographic`, Sentry wiring, centralized middleware-based route protection (currently per-page/layout `auth()` checks via `requireSession()`/`getApiUserId()` — fine for now, revisit if it becomes repetitive).
...
- [ ] Playwright E2E suite (`apps/web/e2e/`) is not wired into CI yet — it needs the worker running alongside the existing Postgres service container, which the current `ci.yml` doesn't set up. Documented in `playwright.config.ts` and here as a real follow-up, not an oversight; run manually (`pnpm --filter web test:e2e` against a locally running full stack) until then.
...
- **`supabase login` / `vercel login` / `render login` all need a TTY this sandbox doesn't have** (device-code/browser callback flows fail with e.g. `LegacyLoginMissingTokenError`). Workaround used for all three: generate a personal access token from each dashboard (Supabase: Account Settings → Access Tokens; Vercel: Account Settings → Tokens; Render: Account Settings → API Keys) and pass it per-command as `SUPABASE_ACCESS_TOKEN=...` / `VERCEL_TOKEN=...` / `RENDER_API_KEY=...`, never via an interactive login step.
...
- **Playwright E2E suite exists but isn't in CI** (see Remaining Tasks) — it's a real, working suite, just not yet automated on every push. Don't assume a green CI run means the reader flows were re-verified; they were verified manually this phase and need to be re-run manually until the CI gap is closed.
...
# E2E (Playwright) — needs web + worker + local Postgres already running
# (not wired into CI yet, see Remaining Tasks)
