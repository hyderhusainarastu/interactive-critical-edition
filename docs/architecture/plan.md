...
2. **Library, identity, and source coverage — complete 2026-07-21 (release-gated).** `learning_resource.normalized_key` continues to canonicalize DOI/ISBN/canonical URL/title-author-year identities; Library read paths aggregate source roles rather than duplicating source rows. Additive migration `0020_phase_12_library_identity` supplies an indexed verified SHA-256 document hash for duplicate detection, with **Open existing** / **Add as another edition** resolution. Source rows expose type, peer-review state, authority, creator verification, and credibility. Public Reddit results remain Tavily/web-search metadata (snippet + outbound URL only), classify as social/E, and have no direct adapter or scraping. Tavily authenticates with its bearer header; an enabled provider omitted by every generated discovery lane is labelled **not selected** in the reader rather than misreported as an outage. Blogger is deliberately limited to a configured public `BLOGGER_BLOG_IDS` allowlist because its API searches only inside known blogs; it shares the existing 12-result blog/newsletter web budget with Tavily. Mastodon status search enables only with both instance URL and access token; Bluesky requires both account identifier and app password. Cumulative-with-exact reader-level matching now covers Library, Roadmap, Curriculum, and published-reader annotations. The behavior stays behind `PHASE_12_LIBRARY_IDENTITY_ENABLED=false` until the production migration and release approval; it was migrated and tested locally only, with no paid processing.
3. **Pipeline v4 and annotations — complete 2026-07-21 (release-gated).** `ANALYSIS_PIPELINE=v4` is parsed alongside retained v2/v3 rollback paths, but the worker downgrades to v3 unless `PHASE_12_PIPELINE_V4_ENABLED=true`. Additive migration `0021_shallow_nova` adds scoped `helpful_for` passage metadata, source-authored apparatus, grounded work claims, compact embedding cache, and unjudged relationship candidates. V4 splits structural body text into at most eight section-aware 12,000-character chunks, deduplicates overlapping grounded annotations, persists footnotes/endnotes/bibliography/citation blocks independently, uses resolved title/author in all synthesis, and records per-call cost. A protected `/api/admin/pipeline-v4/backfill-forecast` plus the Admin dashboard supplies a read-only cost forecast; there is deliberately no backfill execution until separately approved. Migration and tests ran locally only; no production migration, v4 canary, or paid backfill has occurred.
4. **Interactive reader — complete 2026-07-21 (release-gated).** Additive migration `0022_purple_black_bolt` adds canonical `note_highlight` joins (with legacy-link backfill), suggested/verified term pairs, and anchored term occurrences. With `PHASE_12_INTERACTIVE_READER_ENABLED=true`, the default edition surface is labelled **Processed text** and source choices are **Original PDF**/**Original text**; it adds an outline, persisted page/block position, cumulative/exact reader levels, type/relationship/provenance filters, previous/next anchored annotations, independently filtered author apparatus, stable split controls, and focus mode with the shell exit retained. Selection actions create highlights, start linked notes, or attach existing notes. Only approved term pairs follow the global script preference and carry language/direction/accent metadata in processed text; original PDFs remain unchanged. New routes authenticate and enforce ownership, and every new table access stays dormant until the flag is enabled. Local migration, DB/web typechecks, lint, diff check, and web build pass; production migration/enablement and browser acceptance remain pending.
