...
Resolving Concern #1 above: **the production judge for the anthropic rung of `claim_relationship_judgment` runs RAW-TEXT-VALIDATED mode** — claude-haiku-4-5, the BASELINE judge prompt, called as raw text ("Return ONLY valid JSON") rather than forced tool-use. This is the only config, across all of Phase 25.5/25.5b/25.5c (10 structured configs and this one raw-text config), that clears every quality gate: pooled macroF1 0.752 (>=0.75), kappa 0.650 (>=0.60), contradiction recall 1.000 (>=0.66) on the 42-pair empirical gold set, and (PROVISIONAL) macroF1 0.795/kappa 0.688/contradictionRecall 1.000 on the 36-pair humanities set. The report's own initial decision — ship the best *structured* config (Cell 1, macroF1 0.733, missing the gate by 0.017) and leave raw-text as eval-only — is overridden by this moderator decision, made with the report's own concerns above in view, not despite them.

**Why this override is judged safe, addressing the report's own Concern #3 (structural durability) directly:** the gap between "eval-only fence-strip helper" and "production-safe client" was never really about output mode per se — it was about whether a malformed raw-text response degrades gracefully. `AnthropicTextJsonClient` (`packages/ai-adapters/src/anthropicTextJson.ts`) closes exactly that gap: it retries a parse-or-validation failure up to `MAX_RETRIES` (2) times, identically to `AnthropicStructuredClient`/`OpenAIResponsesClient`, and on exhaustion returns a typed `{ ok: false }` failure instead of throwing or ever returning invented data — the caller skips the pair, the same fail-closed contract every other production adapter call in this codebase already makes. For a judge schema whose every field is a closed enum (`relationship`, `category`, `strongerEvidence`) or a length-bounded string, `validate()` running *after* `JSON.parse()` enforces materially the same shape guarantee a JSON-schema/forced-tool-use gate enforces *before* the model replies — the sequencing differs, the correctness guarantee for this specific schema shape does not. This is a deliberate, narrow, eval-driven deviation from the structured-output-everywhere default (plan §33) for exactly one task/provider pairing (`claim_relationship_judgment` × anthropic) where the evidence is unambiguous (0.752/0.650/1.000, clearing every gate, on a 42-pair pooled set — not a single lucky run), not a general relaxation of the structured-output rule elsewhere in the codebase. Every other task route (`relationship_classification`, `metadata_extraction`, `citation_parse`, `claim_extraction`, `debate_cluster_naming`, `evidence_chamber_synthesis`, `hypothesis_generation`) is unaffected and continues to use forced tool-use/JSON-schema structured clients exclusively.

**Implementation:** `AnthropicTextJsonClient` in `packages/ai-adapters/src/anthropicTextJson.ts`, unit-tested (`anthropicTextJson.test.ts`: happy path, fenced-output stripping, parse-fail retry, validation-fail retry, retry exhaustion returning a typed failure, 429/5xx retry, definitive-4xx fail-fast, no-key fail-fast) and exported from the package's `index.ts`. `packages/ai-adapters/src/routing.ts`'s `TASK_ROUTES.claim_relationship_judgment` comment is updated to record this decision and to direct a future production caller at `AnthropicTextJsonClient` specifically (not `AnthropicStructuredClient`) for the anthropic rung; the openai rung is unchanged (still `OpenAIResponsesClient` structured mode via `judge.ts`'s `JUDGE_OUTPUT_SCHEMA`). Concern #4 (the `mechanism` field present in the shipped schema but absent from Cell 1's measured schema) does not apply to raw-text mode — the raw-text config measured used the exact BASELINE prompt text verbatim, with no JSON-schema `required` list to reconcile, so there is no equivalent generalization gap to flag here.

This task type still has no production caller as of this decision (same grep-confirmed absence the report itself notes) — the decision fixes which client a future caller must use, it does not itself add one.
