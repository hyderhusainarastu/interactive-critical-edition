...
## Moderator gate decision (2026-07-26)

**Promotion DENIED.** Three of four floors fail outright (`HUMANITIES_BRANCH_DELTA_MIN` 0.033 vs. 0.05; `MECHANISM_ACCURACY_MIN` 0.364 vs. 0.60; `EMPIRICAL_REGRESSION_MAX` 0.032 vs. 0.02), and the report's own nuance clause does not rescue the result — that clause anticipated only the single delta-floor miss, not a broader three-of-four shortfall. Migration `0046` does not ship; the humanities-branch judge config (discipline preamble + mechanism field) stays out of production. Humanities-domain claim pairs continue to be scored by the base judge (`branch: "empirical"`), which this gate itself measured at macroF1 0.771/kappa 0.653/contradiction recall 1.000 on the ratified humanities set — a strong result in its own right, with perfect contradiction recall and no class below the F1 floor.

**Mechanism accuracy is the substantive failure**, not a rounding-margin miss: 6 of the 7 wrong predictions collapse `different_scope_conditions`/`different_definition` into a catch-all `interprets_differently`, a systematic under-differentiation between the three stage-2 mechanisms rather than noise, on a genuinely thin n=11 gold sample. This is the floor missed by the widest margin (0.236 below bar) and is judged the primary reason promotion is denied, independent of the other two misses.

**The empirical-regression finding (0.032 drop) additionally mandates strict branch routing if this is ever revisited**: turning on the humanities-branch code path for domain-appropriate data is fine on its own, but the measured regression on empirical-domain data confirms that any future call site routing data through the humanities branch must be strict and well-tested — a mis-routed empirical pair through the humanities branch would measurably underperform the certified config. This requirement carries forward into any future attempt at this gate, not just as a Concern.

**Future path, if this is revisited:** mechanism-targeted few-shot examples distinguishing `different_scope_conditions`/`different_definition`/`interprets_differently` specifically (rather than a generic instruction), plus a larger mechanism gold set — n=11 is too thin for a stable 3-way-plus-null classification measurement, where a single flip moves accuracy by roughly 0.09. No re-run is scheduled; this stays a documented future-work item, not a committed next phase.
