...
No rung passed. Per the plan's re-plan clause, a prompt-iteration pass (2c treatment: decision tree + boundary few-shots, train-split tuning) was dispatched before any tier escalation — see `spike-25-5b-judge-iteration.md`. The `none_detected` engagement framing will be dropped (2/10 measured flips prove non-neutrality); engagement context will be supplied only when engagement exists.
