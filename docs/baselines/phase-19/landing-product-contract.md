...
- **Lead paragraph** (muted text): "Hover any marker and see what a passage references, why, and how sure the system is — with the exact source text that triggered it. Approve, edit, or dismiss anything. Original footnotes, AI annotations, and your own notes stay visually distinct."
...
`apps/web/e2e/landing-contract.spec.ts` — 4 Playwright `toHaveScreenshot()` assertions (Reader/Annotations × desktop/mobile, Roadmap × desktop/mobile) against the checked-in Linux baselines in `apps/web/e2e/landing-contract.spec.ts-snapshots/`. Not yet wired into the CI-safe spec list (`.github/workflows/ci.yml`'s `E2E (CI-safe specs)` step) — deferred until the rest of the Phase 19 audit is complete and this becomes a running gate rather than a one-off capture, per the plan's own sequencing (freeze the contract now, enforce it starting Phase 22 when authenticated-surface parity work begins).
