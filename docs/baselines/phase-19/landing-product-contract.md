...
> **SUPERSEDED 2026-07-23, with owner sign-off.** The owner directed a full landing-page rebuild from their own campaign site (`/Users/hyderhusainarastu/Project/palimnote-campaign`): a new hero, the campaign theme, campaign-derived annotated-reader and Library depictions, the campaign's interactive 3D knowledge graph carried over unchanged, and a new Ask Library section. The two sections this document froze — `ReaderShowcase()` and `RoadmapShowcase()` in `apps/web/src/app/page.tsx` — no longer exist, so the Phase 19.4 freeze is *superseded*, not broken. Everything below is retained as the historical record of what the authenticated Reader/Annotations/Roadmap surfaces were brought into parity with during Phase 22; that parity work is unaffected, since it changed the app, not the landing page.
>
> **The live contract now lives in `apps/web/e2e/landing-contract.spec.ts`** and guards the two depictions the owner singled out as the ones to keep: the annotated reader (`reader-annotations-{desktop,mobile}-chromium-linux.png`) and the Library (`library-{desktop,mobile}-chromium-linux.png`). The knowledge-graph section is deliberately excluded — it renders a canvas whose projection is recomputed per device-pixel-ratio and container size, so it is not a stable screenshot subject; `landing.spec.ts` asserts it structurally instead. Baselines were regenerated through the same pinned `mcr.microsoft.com/playwright:v1.61.1-noble` image, against a production `next start` build, and verified pixel-stable across three consecutive runs. The mirror PNGs in this directory were replaced to match; the `landing-roadmap-*.png` mirrors were deleted along with the section they depicted.
