...
2. `SELF_SIGNED_CERT_IN_CHAIN` at startup, specifically inside `pg-boss`'s `Contractor.isInstalled`/`PgBoss.start` (R-19-2) — a `DATABASE_URL` rotation (new password, refreshed pooled connection string) plus a `pg-connection-string` version that now maps `sslmode=require`/`prefer`/`verify-ca` to `verify-full`. This is **specific to pg-boss's own internal `pg` client** — `packages/db`'s Drizzle client uses `postgres.js` and is unaffected, so `pnpm --filter @ice/db db:migrate` working fine is not evidence the worker will boot. Fix shape: `packages/db/src/queue.ts`'s `buildPgBossConfig()` must parse `DATABASE_URL` into discrete `host`/`port`/`database`/`user`/`password` fields (no `connectionString` key) so an explicit `ssl: { rejectUnauthorized: false }` isn't silently overwritten by node-postgres's own connection-string parsing (`ConnectionParameters`'s `Object.assign` spreads the parsed value last). **After any Supabase DB password/connection-string rotation, redeploy the worker and check its startup log for this string before assuming pg-boss is fine just because the web app's DB queries work.**
3. `ECIRCUITBREAKER` / auth failures right after a DB password reset — transient, retry ~20s apart for up to ~2 minutes (documented, not a bug).
4. `ENETUNREACH` on `db.<ref>.supabase.co` — wrong connection string; the worker must use the Supavisor pooler (`aws-0-us-east-1.pooler.supabase.com:6543`), never the direct IPv6-only host.
5. Any crash now reaches `reportError` before the process exits (Phase 19 addition to `apps/worker/src/index.ts`) — check Sentry (if `SENTRY_DSN` is configured) or the structured `[reportError]` log line first; it carries `scope`, the resolved pipeline version, and the deploy commit. Note this only covers crashes *inside* job handlers or the top-level process — pg-boss's own startup failure (R-19-2) happens before `reportError`'s handlers are even installed, so a startup crash-loop must be diagnosed from `render logs` directly, not from Sentry.
