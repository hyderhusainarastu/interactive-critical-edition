 ...
 *
 * Both bakeoff prototypes (and, later, the real Knowledge Map rebuild)
 * consume exactly this module for band placement and camera pose so
 * Prototype A and Prototype B are judged on rendering/interaction, not on
 * two different reimplementations of the same geometry deciding the
 * outcome. Every export here is a pure function of its inputs — no DOM,
 * no three.js, no React, no mutable module state — precisely so it is
 * unit-testable with vitest without a WebGL context.
 *
...
/** Charter §8: BAND_GAP = min(120, max(48, 1.25 × median XY link distance)). */
export const BAND_GAP_MIN = 48;
export const BAND_GAP_MAX = 120;
export const BAND_GAP_MEDIAN_MULTIPLIER = 1.25;
...
// ---------------------------------------------------------------------
// Semantic band Z assignment (charter §8)
// ---------------------------------------------------------------------

/** BAND_GAP = min(120, max(48, 1.25 × median XY link distance)). */
export function computeBandGap(medianXYLinkDistance: number): number {
  return Math.min(BAND_GAP_MAX, Math.max(BAND_GAP_MIN, BAND_GAP_MEDIAN_MULTIPLIER * medianXYLinkDistance));
}

/** A node's semantic Z position: bandIndex × BAND_GAP. `bandIndex` is one of
 * the charter's six indices, -2..3. */
export function bandZ(bandIndex: number, bandGap: number): number {
  return bandIndex * bandGap;
}

/** Charter §8: "Within-band Z jitter may not exceed 0.08 × BAND_GAP and
 * carries no meaning." Returns the maximum allowed jitter magnitude. */
export function maxBandJitter(bandGap: number): number {
  return 0.08 * bandGap;
}
