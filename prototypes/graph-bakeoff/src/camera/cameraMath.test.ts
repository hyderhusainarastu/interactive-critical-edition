...
describe("band Z assignment (charter §8)", () => {
  it("clamps BAND_GAP to the [48, 120] range", () => {
    expect(computeBandGap(0)).toBe(BAND_GAP_MIN);
    expect(computeBandGap(1000)).toBe(BAND_GAP_MAX);
  });
  it("uses 1.25 × median when that falls inside the clamp range", () => {
    // median 60 -> 1.25*60 = 75, inside [48,120]
    expect(computeBandGap(60)).toBeCloseTo(75, 10);
  });
  it("computes z = bandIndex * BAND_GAP for every one of the six charter bands", () => {
    const gap = 80;
    for (const bandIndex of [-2, -1, 0, 1, 2, 3]) {
      expect(bandZ(bandIndex, gap)).toBe(bandIndex * gap);
    }
  });
  it("bounds within-band jitter to 0.08 × BAND_GAP", () => {
    expect(maxBandJitter(100)).toBeCloseTo(8, 10);
  });
});
